...
- ### 上周进度
	...
	- 一个比较浪费时间的其实是万门备份计划的网站用户系统，本来计划自己开发的，但是发现使用 KV 来实现用户系统真的太难了，并且也不安全，而 Cloudflare 的关系型数据库才刚刚开始内测估计要半年后才能 GA（其实 KV 支持索引也行啊，唉），考虑了下第三方服务的，但是发现无论是 Auth0 还是 Authing 免费版本的限制都过多了，实在没办法。于是
	-
- ### SuperMemo Plan
- ### 并发
-
