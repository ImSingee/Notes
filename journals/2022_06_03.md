- PIPELINE_ERROR_DECODE 问题
	...
	- 解决方案：
		...
		- ```js
		  ...
		  ```
	- 已知问题：因为 Artplayer 的容错设计，所有错误在一个 Artplayer 对象的生命周期内只会被重试 RECONNECT_TIME_MAX 次（默认为 5），记录已经重试次数的是闭包变量外界无法修改，因此建议将这个值修改到足够大，或是建议 Artplayer 开发者将这个参数值暴露出来且在每次调用 switchUrl 时进行重置
-